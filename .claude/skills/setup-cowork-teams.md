...
这是唯一的串行阶段。分为两步：先理解用户目标，再扫描项目。两者共同构成所有并行 Agent 的共享输入。
...
将用户回答整理为**目标摘要**，嵌入后续的 Scan Context 中。这个目标摘要将直接影响：
- CLAUDE.md 的侧重点（Agent A 用它来决定哪些偏好最重要）
- Context files 的提取方向（Agent C 用它来决定提取什么领域知识）
- Plugin skills 的选择（Agent D 用它来生成最相关的 skills）
- .claudeignore 的排除策略（Agent B 用它来判断什么是"噪音"）
