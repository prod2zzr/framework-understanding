...
2. `--here` → 项目创建在**当前目录内**（`./[项目名]/`）而非默认的 `../[项目名]/`。适用于 `--remote` 模式或无法在项目目录外创建文件夹的环境
3. 自然语言关键词自动推断模板：
...
**--remote 模式示例**（Code --remote 或受限环境下使用）：
- `/quickstart 调研 AI Agent 框架 --here` → 在当前目录内创建项目
- `/quickstart myapp --code --here` → 代码项目，建在 `./myapp/`
...
> ⚠️ **--remote 注意**：Code `--remote` 模式下 AskUserQuestion 弹窗不可见，交互会断裂。建议在 --remote 中始终传入完整参数（项目名 + 模板关键词），使用快速模式跳过交互。
...
**路径选择优先级**：
1. 用户指定了路径 → 用指定路径
2. `--here` 参数 → `./[项目名称]/`（当前目录内）
3. 当前目录为空（或仅含 `.git`）→ 直接在当前目录初始化，不新建子目录
4. 默认 → `../[项目名称]/`（当前目录的同级）

**降级逻辑**：如果默认路径 `../[项目名称]/` 创建失败（如 --remote 模式下无权限），自动降级到 `./[项目名称]/`，并提示用户：「当前环境无法在项目目录外创建文件夹，已在当前目录内创建。」
