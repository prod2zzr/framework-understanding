...
- 严格遵守 token 预算约束
- 遇到已有配置文件时，先询问用户再操作
- **Compact 防护**：在 Phase 2（CLAUDE.md 生成）完成且用户确认后，主动执行 `/compact`，焦点为"保留用户目标摘要、已确认的 CLAUDE.md 内容要点、以及待执行的 Phase 3-6 任务清单"。这将清理 Phase 1 扫描过程中的冗长工具输出，为后续阶段释放上下文空间
...
**等待用户回复。** 将回答整理为目标摘要，用于指导后续所有阶段的生成方向。
...
将五项结果整理为结构化摘要（含用户目标摘要），然后 **停下来请用户确认**：
> "以上是你的目标理解和项目扫描结果。请确认：
> 1. 我对你的工作目标理解是否准确？
> 2. 哪些是核心工作区？
> 3. 哪些是归档区（不需要 Claude 读取）？
> 4. 有没有敏感区域需要特别排除？
> 确认后我将进入 Phase 2。"
