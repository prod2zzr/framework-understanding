...
## 代码质量标准：事前检查（Pre-validation）
execute_pandas 内置了自动列名预校验机制，这是本项目的质量标准之一：
- 你编写的代码中引用的列名（df['xxx']）会在执行前自动与 DataFrame 实际列名比对
- 如果引用了不存在的列，代码不会执行，而是立即返回错误信息、实际可用列名和样本数据
- 你应根据返回的 available_columns 和 sample_data 修正代码后重试
- 每次成功执行后，结果中都附带 dataframe_meta（列名、类型、行列数），帮助你编写后续代码
- 因此，第一次编写代码时务必使用 parse_excel 返回的实际列名，避免触发预校验错误
